...
void IVP_SurfaceBuilder_Q12::swap_bsp_data()
{
	...
	for (j=0 ; j<3 ; j++) {
	    d->mins[j] = LittleFloatPlatform(d->mins[j]);
	    d->maxs[j] = LittleFloatPlatform(d->maxs[j]);
	    d->origin[j] = LittleFloatPlatform(d->origin[j]);
	}
    }
    ...
    for (i=0 ; i<this->n_planes ; i++) {
	for (j=0 ; j<3 ; j++) {
	    this->dplanes[i].normal[j] = LittleFloatPlatform (this->dplanes[i].normal[j]);
	}
	this->dplanes[i].dist = LittleFloatPlatform (this->dplanes[i].dist);
